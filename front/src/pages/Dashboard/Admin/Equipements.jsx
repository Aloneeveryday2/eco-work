...
export default function Equipements() {
  ...
  useEffect(() => {
    ...
    const fetchEquipements = async () => {
      try {
        ...
        if (res.ok) {
          setEquipements(Array.isArray(res.data) ? res.data : []);
        } else {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setEquipements(e => Array.isArray(e) ? [...e, res.data] : [res.data]);
        setNewLibelle("");
      } else {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setEquipements(e => Array.isArray(e) ? e.map(x => x.id === editEq.id ? res.data : x) : [res.data]);
        setEditEq(null);
      } else {
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "280px 1fr", gap: "1.5rem", alignItems: "start" }}>
        ...
        <div style={{ background: "white", borderRadius: "16px", overflow: "hidden", boxShadow: "0 1px 12px rgba(26,58,69,0.06)" }}>
          ...
          {safeEquipements.map((eq, i) => (
            <div key={eq.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.9rem 1.5rem", borderBottom: i < safeEquipements.length - 1 ? "1px solid rgba(26,58,69,0.04)" : "none" }}>
