  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setShowCreate(false);
        setNewAdmin({ nom: "", prenom: "", email: "", telephone: "", adresse_postale: "", password: "", type_de_compte: "admin" });
      } else {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              background: "white",
              border: "1px solid rgba(26,58,69,0.1)",
              borderRadius: "8px",
                ...
                <td style={{ padding: "1rem 1.2rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
                    <div style={{
                      ...
                      height: 34,
                      borderRadius: "50%",
                      background: u?.type_de_compte === "admin" ? "rgba(123,223,242,0.15)" : "#f0f4f5",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      fontSize: "0.78rem",
                      fontWeight: 700,
                      color: u?.type_de_compte === "admin" ? "#1a3a45" : "#4a7a85"
                    }}>
                      {u?.prenom?.[0]}{u?.nom?.[0]}
                    </div>
                    <div>
        ...
        <Modal title="Modifier l'utilisateur" onClose={() => setEditUser(null)}>
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <div style={{ display: "flex", gap: "0.8rem", justifyContent: "flex-end" }}>
              ...
              <Btn variant="cyan" onClick={handleUpdate} disabled={isSubmitting}>{isSubmitting ? 'Enregistrement...' : 'Enregistrer'}</Btn>
            </div>
          </div>
        </Modal>
      )}
        ...
        <Modal title="Créer un administrateur" onClose={() => setShowCreate(false)}>
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <div style={{ display: "flex", gap: "0.8rem", justifyContent: "flex-end" }}>
              ...
              <Btn variant="cyan" onClick={handleCreate} disabled={isSubmitting}>{isSubmitting ? 'Création...' : 'Créer'}</Btn>
            </div>
          </div>
        </Modal>
      )}
      ...
      {deleteConfirm && (
        <Modal title="Confirmer la suppression" onClose={() => setDeleteConfirm(null)}>
          ...
          <div style={{ display: "flex", gap: "0.8rem", justifyContent: "flex-end" }}>
            ...
            <Btn variant="danger" onClick={handleDelete} disabled={isSubmitting}>{isSubmitting ? 'Suppression...' : 'Supprimer'}</Btn>
          </div>
        </Modal>
      )}
    </div>
