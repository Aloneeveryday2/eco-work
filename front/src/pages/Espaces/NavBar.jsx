...
export default function NavBar() {
  ...
  return (
    <nav style={{
      ...
      <div style={{ display: 'flex', gap: isMobile ? '0.2rem' : '0.5rem', alignItems: 'center' }}>
        ...
        {isLoggedIn ? (
          <button onClick={handleLogout} style={{
            ...
            padding: isMobile ? '0.4rem 0.8rem' : '0.5rem 1.2rem', borderRadius: '100px', cursor: 'pointer',
          }}>
            {isMobile ? 'Déconnexion' : 'Se déconnecter'}
          </button>
        ) : (
          <>
            <Link to="/login" style={{ textDecoration: 'none', color: '#4a7a85', fontSize: isMobile ? '0.75rem' : '0.85rem', padding: isMobile ? '0.4rem 0.6rem' : '0.5rem 1rem' }}>
              {isMobile ? 'Login' : 'Connexion'}
            </Link>
            <Link to="/inscription" style={{
              textDecoration: 'none', background: '#1a3a45', color: '#eff7f6',
              fontSize: isMobile ? '0.72rem' : '0.82rem', fontWeight: 600, padding: isMobile ? '0.4rem 0.8rem' : '0.5rem 1.2rem', borderRadius: '100px',
            }}>
